...
[F] [G] [H] [Z] [N] [P] [M] [N] [D]
 1   2   3   4   5   6   7   8   9
    ...
    while (getline(cin, input)) {
        ...
        // pop quantity items from source stack to target stack one by one.
        for(int j = 0; j < quantity; j++) {
            char ch = stacks[source - 1].top();
            stacks[source - 1].pop();
            stacks[target - 1].push(ch);
        }
    }
    // answer will be top of all stacks from left to right.
    string ans = "";
